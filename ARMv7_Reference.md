...
|:------------|:-------|:------|
| `MOV Rd, Operand2` | Rd := Operand2 | Register to register, or a small constant. |
| `MVN Rd, Operand2` | Rd := NOT Operand2 | Bitwise complement. |
| `LDR Rd, =value` | Rd := any 32-bit constant | Use whenever MOV rejects the constant as too large. |
| `LDR Rd, =label` | Rd := address of label | How you get the address of a variable or array. |
...
| `ADC Rd, Rn, Operand2` | Rd := Rn + Operand2 + C | Multi-word addition. |
| `SUB Rd, Rn, Operand2` | Rd := Rn − Operand2 | |
| `SBC Rd, Rn, Operand2` | Rd := Rn − Operand2 − NOT C | Multi-word subtraction. |
| `RSB Rd, Rn, Operand2` | Rd := Operand2 − Rn | Reverse subtract. `RSB Rd, Rn, #0` negates Rn. |
| `MUL Rd, Rm, Rs` | Rd := Rm × Rs | Low 32 bits only. Both operands must be registers — no immediate. |
| `MLA Rd, Rm, Rs, Rn` | Rd := (Rm × Rs) + Rn | Multiply-accumulate. |
| `UMULL RdLo, RdHi, Rm, Rs` | 64-bit product | SMULL for signed operands. |
...
| Instruction | Effect | Notes |
|:------------|:-------|:------|
| `AND Rd, Rn, Operand2` | Rd := Rn AND Operand2 | Masking: keep selected bits. |
| `ORR Rd, Rn, Operand2` | Rd := Rn OR Operand2 | Set selected bits. |
| `EOR Rd, Rn, Operand2` | Rd := Rn XOR Operand2 | Toggle bits. `EOR Rd, Rd, Rd` clears Rd. |
| `BIC Rd, Rn, Operand2` | Rd := Rn AND NOT Operand2 | Bit clear: clear selected bits. |
| `LSL Rd, Rm, #n` | Rd := Rm << n | Multiply by 2ⁿ. |
| `LSR Rd, Rm, #n` | Rd := Rm >> n, zero fill | Unsigned divide by 2ⁿ. |
| `ASR Rd, Rm, #n` | Rd := Rm >> n, sign fill | Signed divide by 2ⁿ. |
| `ROR Rd, Rm, #n` | Rotate right by n | Bits leaving the bottom re-enter at the top. |
...
| Instruction | Effect |
|:------------|:-------|
| `LDR Rd, [Rn]` | Load the 32-bit word at address Rn into Rd. |
| `STR Rd, [Rn]` | Store the word in Rd to address Rn. |
| `LDRB / STRB` | Byte transfer. `LDRB` zero-extends, `LDRSB` sign-extends. |
| `LDRH / STRH` | Halfword transfer. `LDRSH` sign-extends. |
| `PUSH {reglist}` | Push registers onto the stack, e.g. `PUSH {R4, LR}`. |
| `POP {reglist}` | Pop registers off the stack. |
